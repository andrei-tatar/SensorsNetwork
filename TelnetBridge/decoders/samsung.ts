export class SamsungDecoder {
    ...
    decode(pulses) {
        if (pulses.length != 67 || !this.matches(pulses[0], SamsungDecoder.header_mark) || !this.matches(pulses[1], SamsungDecoder.header_space))
            return null;
    ...
    encode(code) {
        ...
        var pulses = [SamsungDecoder.header_mark, SamsungDecoder.header_space];
        for (var i = 31; i >= 0; i--) {
